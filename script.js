  ...
  xhrStatus.addEventListener("readystatechange", () => {
    ...
    if (xhrStatus.readyState === 4) {
      ...
      for (let line = 0; line < outputTextStatus.length; line++) {
        ...
        const tableCellLeftText = document.createTextNode(
          outputTextStatus[line].name
        );
        ...
        for (
          let lineStatus = 0;
          lineStatus < outputTextStatus[line].lineStatuses.length;
          lineStatus++
        ) {
          const tableCellRight = document.createElement("td");
          const tableCellRightText = document.createTextNode(
            outputTextStatus[line].lineStatuses[lineStatus]
...
function journeyPlanner(form) {
  ...
  if (!regex.test(fromPostcode)) {
    addPostcodeWarning(
      "Make sure the postcodes you have entered are valid London and Greater London postcodes."
    );
    return;
  }
  ...
  xhrJourney.addEventListener("readystatechange", () => {
    if (xhrJourney.readyState === 4) {
      ...
      addJourneySteps(
        `Leave at ${journeyStartTime.slice(
          journeyStartTime.indexOf("T") + 1,
          journeyStartTime.length - 3
        )} to arrive at ${journeyEndTime.slice(
          journeyEndTime.indexOf("T") + 1,
          journeyEndTime.length - 3
        )}.`
      );
...
document.addEventListener(
  ...
  function(event) {
    if (
      event.target.attributes["name"].value == "fromPostcode" ||
      event.target.attributes["name"].value == "toPostcode"
    ) {
...
const getJourney = () => {
  ...
  xhrJourney.addEventListener("readystatechange", () => {
    if (xhrJourney.readyState === 4) {
      let outputTextJourney = JSON.parse(xhrJourney.responseText).journeys["0"];
      addJourneySteps(
        `Leave at ${outputTextJourney.startDateTime} to arrive at ${outputTextJourney.arrivalDateTime}. Duration = ${outputTextJourney.duration}`
      );
...
const initJourneyContainer = () => {
  const sectionJourneyContainer = document.getElementById("journeyContainer");

  const h2 = document.createElement("h2");
  const h2Message = document.createTextNode("Optimal Route");
  h2.appendChild(h2Message);

  const divRouteContainer = document.createElement("div");
  divRouteContainer.setAttribute("id", "routeContainer");

  sectionJourneyContainer.appendChild(h2);
  sectionJourneyContainer.appendChild(divRouteContainer);
};
